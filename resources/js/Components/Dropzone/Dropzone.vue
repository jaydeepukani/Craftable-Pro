...
const onFilesDrop = (e: any) => {
  const acceptedFiles = props.accept?.split(",").map((itm) => itm.trim());

  const allAccept = Array.from(e.dataTransfer.files).every((file) => {
  ...
  if (allAccept) {
    ...
    dropIsActive.value = false;
  } else if (!allAccept && props.accept !== undefined) {
    toast.error(
      trans(
        "craftable-pro",
        "You cannot upload this file type. Only the following types are allowed: " +
          acceptedFiles,
      ),
    );
  }
